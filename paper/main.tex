...
sequence $k_n/(2n)\to\rho\in(0,1)$, and translate the result to matchgate
complexity.  If $S$ and $T$ are independent uniform $k_n$-subsets of
$[2n]$, write
...
$\min\{s,t\}-st$.  In the central component $k_n=n$, the limiting coefficient
is $2$ and the mean constant is $\sqrt{2\pi}/4$, resolving Conjecture~14 of
West et al. after the matchgate--minuscule-lattice identification.  We also
record the variance asymptotic, extract the leading finite-size correction
from the published closed form, and give a finite sum obtained by conditioning
on the symmetric-difference size.  For complexity, we derive the joint
vertex-and-Haar graph-complexity mean and prove the pointwise bound from graph
complexity to Krylov complexity for generator-supported time-independent
Hamiltonians.
...
The commutator graph was introduced by Diaz et al.~\cite{DiazEtAl}.
...
$\alpha>0$ fixed.  Taking $\alpha=\rho/(1-\rho)$ gives the same limiting law,
mean, and variance constants as those stated below; at $\alpha=1$ it gives the
central case.  Thus their theorem, combined with the matchgate component
identification of West et al., already implies Conjecture~14.  The present
finite-population proof treats the triangular-array extension in which the two
rectangle dimensions may vary arbitrarily subject only to
$k_n/(2n)\to\rho$, equivalently $k_n/(2n-k_n)\to\alpha$, and supplies direct
moment bounds.  It also derives the Haar matchgate specialization, gives a
...
Table~\ref{tab:provenance} separates the published suppliers from the
deductions and alternative arguments given here.
...
& Finite-population proof here for every $k_n/(2n)\to\rho$, extending the
fixed-ray formulation and giving direct second- and fourth-moment bounds.\\
Exact finite mean
...
\]
For each $1\leq a<2n$, put $g_a:=-ic_ac_{a+1}$.  The displayed identities give
$g_{2j-1}=Z_j$ and $g_{2j}=X_jX_{j+1}$, so $g_a\in\Gamma_n$.  Moreover,
...
\begin{corollary}[Joint vertex-and-Haar matchgate graph complexity]\label{cor:complexity}
...
\]
This fixed-vertex mean can depend on $P_n$.  Averaging also over $P_n$ gives
exactly $\E D_{2n,k_n}$, so
...
West et al.~\cite[Eqs.~(A81)--(A82)]{WestEtAl} does not follow from the stated
...
\subsection{Published rectangular mean}
...
\subsection{Exact finite-sum evaluation}
...
identified by the Git tag \texttt{paper-2026-09-01-r2} at
\url{https://github.com/skypher/matchgate/tree/paper-2026-09-01-r2}.  It is
...
The Electronic Journal of Combinatorics \textbf{31} (2024), Paper No.~P1.41.
