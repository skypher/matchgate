...
\hypersetup{
  ...
  pdftitle={Brownian-Bridge Scaling of Graph Complexity in Matchgate Dynamics},
...
\title{Brownian-bridge scaling of graph complexity\\
in matchgate dynamics}
...
the limiting law and its expectation constant.
...
\cite{DeAngelisGray}.  Brownian-bridge limits for $L^1$ empirical-process and
...
The contributions established here are the application to the matchgate
component and Conjecture~14, the fixed-density finite-population law and
variance, and the exact central-component formula and finite-size correction in
Section~6.
...
between the two cumulative distribution functions~\cite{DeAngelisGray}.
...
\texttt{check-exact-mean} target replays every recorded row and fails if an
output leaves its stated numerical tolerance.
