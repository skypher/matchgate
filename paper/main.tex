...
Write $\mathsf G_P(U^\dagger P U)$ for graph complexity based at the Pauli
vertex $P$, in the notation of West et al.~\cite[Eq.~(37)]{WestEtAl}.
...
matchgate group.  Under the normalization of West et al.,
\[
...
Hamiltonian $H$ whose Pauli terms belong to the generating set used to define
the matchgate commutator graph.  Put
\[
 P_t=e^{iHt}Pe^{-iHt},
\]
and let $\mathsf K_{H,P}(t)$ be the Krylov complexity constructed from the
Liouvillian $\mathcal L_H=[H,\,\cdot\,]$ and initial operator $P$.  Then, for
every $t\in\mathbb R$,
\[
...
\begin{proof}
This is West et al.~\cite[Lemma~9 and Eqs.~(A78)--(A86)]{WestEtAl}.  Their proof
uses the Krylov basis generated by $\mathcal L_H$ and the fact that the $j$th
Krylov vector can have Pauli support only at graph distance at most $j$ from
$P$.  The expectation statement follows by integrating the pointwise
inequality.
\end{proof}
...
\label{tab:numerics}
\end{table}
Here $c=\sqrt{2\pi}/4=0.626657068657750\ldots$.  The values are deterministic
floating-point evaluations of finite sums, not Monte Carlo estimates or
...
\texttt{Makefile}.  It is built and run, for example, with
\[
