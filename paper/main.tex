...
obtained by conditioning on the symmetric-difference size.  For graph
complexity, we derive the corresponding asymptotic under joint averaging over
a uniform initial vertex and an independent Haar matchgate unitary.  We also
give a pointwise graph--Krylov comparison for time-independent Hamiltonians in
the real span of the nearest-neighbor matchgate generators.
...
The commutator graph describes how a chosen family of Hamiltonian generators
connects operators.  Its vertices are Pauli strings, the tensor products of
single-site Pauli matrices and identities.  Two vertices are adjacent when
commutation with one of the generators maps one string to a nonzero scalar
multiple of the other.  Graph distance therefore counts the minimum number
of such transitions between operators.  This construction was introduced by
Diaz et al.~\cite[Supplemental Definition~6]{DiazEtAl}.
We study the distance between two uniformly sampled vertices in a matchgate
component.  Its mean also determines graph complexity after averaging over
both a uniform initial operator and an independent Haar matchgate unitary,
as stated in Corollary~\ref{cor:complexity}.
...
The fixed-aspect-ratio theorem of Defant et al., combined with the matchgate
component identification of West et al., already implies Conjecture~14.
We make this implication explicit in Corollary~\ref{cor:conjecture} and give
an independent finite-population proof for arbitrary density sequences
$k_n/(2n)\to\rho\in(0,1)$ in Theorem~\ref{thm:main}, including direct moment
bounds.  We also derive the matchgate graph-complexity consequence and give a
projection proof of the graph--Krylov comparison
(Corollaries~\ref{cor:complexity} and~\ref{cor:krylov}).
...
Lemma~9 is given an independent tail-projection proof here.\\
\end{tabular}
...
\cite{Vallender,DeAngelisGray}.\footnote{Vallender's addendum corrects
Remark~1 and formula~(3) of the original note; it does not alter the CDF-area
identity used here~\cite{VallenderAddendum}.}
...
Barrio, Gin\'e, and Matr\'an~\cite{delBarrioGineMatran}.\footnote{Their
correction~\cite{delBarrioCorrection} concerns separate heavy-tail statements,
...
Section~2 establishes the graph identification and transport identity.
Section~3 supplies the bridge limit and moment bounds used in the distance
theorem of Section~4.  Section~5 derives the graph- and Krylov-complexity
consequences.  Section~6 records the published exact mean
(Proposition~\ref{prop:closed-form}), derives the central bridge-and-gap
identity (Proposition~\ref{prop:central-exact}), and extracts the leading
finite-size correction (Theorem~\ref{thm:finite-size}) before presenting
...
When $N=2n$, let $X_j,Y_j,Z_j$ denote the Pauli matrices acting on site $j$
and the identity on every other site of the $n$-qubit system.  Define the
Jordan--Wigner Majorana operators
\[
...
\begin{remark}[Comparison of normalizations]
Defant et al.~\cite[Proposition~5]{DefantEtAl} state their limit for rectangles
$\mathcal P_{\lfloor\alpha\ell\rfloor,\ell}$ along a fixed ray, with
$\alpha>0$ fixed.  Along this ray, the subset density tends to
$\rho=\alpha/(1+\alpha)$.  Writing $N=k+\ell$ gives
$\ell\sim(1-\rho)N$; on the matchgate subsequence $N=2n$, this becomes
$\ell\sim2(1-\rho)n$.  Converting their $\ell^{3/2}$ normalization to the
present $n^{3/2}$ normalization therefore gives the coefficient
\[
 [2(1-\rho)]^{3/2}\sqrt{2\alpha(1+\alpha)}
 =4\sqrt{\rho(1-\rho)}.
\]
Their moment convergence gives the same mean and variance constants along
these rays; $\alpha=1$ gives the central case.  Theorem~\ref{thm:main}
allows both rectangle dimensions to vary arbitrarily subject to
$k_n/(2n)\to\rho$, equivalently $k_n/(2n-k_n)\to\rho/(1-\rho)$.
\end{remark}
...
The rectangular-lattice interpretation connects matchgate operator distances
to a classical path-area problem.  It makes the implication of Defant et al.'s
theorem for Conjecture~14 explicit, while cumulative counting functions give
an independent route to the distance law for arbitrary density sequences.
The same distance controls graph complexity under joint vertex and Haar
averaging.  For the Hamiltonians in \eqref{eq:matchgate-Hamiltonian}, nested
graph-distance and Krylov subspaces give the pointwise complexity comparison.
The exact finite-size formulas complement these asymptotic and dynamical
statements with directly computable component means.
...
The manuscript source, numerical checker, and recorded outputs are available
at \url{https://github.com/skypher/matchgate}.  The numerical source and
outputs used for Tables~\ref{tab:numerics} and~\ref{tab:noncentral} are
archived at Git tag
\href{https://github.com/skypher/matchgate/tree/paper-2026-09-04-r6}%
{\texttt{paper-2026-09-04-r6}}.  The checker tests the metric and finite-sum
identities by exact rational enumeration in small cases, then reproduces the
recorded floating-point evaluations within stated tolerances.  The tables
were produced with GCC~13.3.0 using \texttt{long double} arithmetic.
Build instructions, software requirements, and the ranges of the exact checks
are documented in the repository's \texttt{README.md}.
