...
formula~\cite{DefantEtAl}.  We make the resulting matchgate implication
explicit, give an independent finite-population derivation valid for every
sequence $\kappa_n/(2n)\to\rho\in(0,1)$, and translate the result to matchgate
complexity.  If $S$ and $T$ are independent uniform $\kappa_n$-subsets of
$[2n]$, write
$D_{2n,\kappa_n}(S,T)=\sum_i|s_i-t_i|$.  Then
\[
 \frac{D_{2n,\kappa_n}}{n^{3/2}}
...
$\min\{s,t\}-st$.  In the central component the limiting coefficient is $2$
and the mean constant is $\sqrt{2\pi}/4$, resolving Conjecture~14 of West et
al. after the matchgate--minuscule-lattice identification.  We also obtain the
variance asymptotic and leading finite-size correction, give a finite sum
obtained by conditioning on the symmetric-difference size, derive the jointly
vertex--Haar-averaged graph-complexity mean, and prove the pointwise bound from
graph complexity to Krylov complexity for generator-supported time-independent
Hamiltonians.
...
identification of West et al., already implies Conjecture~14.  The
contributions specific to the present paper are: (i) making this matchgate
implication explicit; (ii) giving an independent finite-population proof for
every sequence $k_n/(2n)\to\rho$, with direct moment bounds; (iii) deriving the
Haar matchgate specialization and a projection proof of the graph--Krylov
comparison; and (iv) giving a central finite sum obtained by conditioning on
the symmetric-difference size.  The Haar statement is a joint expectation
over an independent uniform initial Pauli vertex and Haar matchgate unitary;
it is not a pointwise-in-vertex scaling statement.
...
used here~\cite{VallenderAddendum}.  Bourn and
Willenbring~\cite{BournWillenbring} study expected one-dimensional earth mover
distance for uniform weak compositions and its probability-simplex limit.  A
stars-and-bars word identifies a uniform $k$-subset of $[N]$ with a uniform
...
Wiener formula of Defant et al. to close the discrete expected-distance
...
simple-walk bridge area and exchangeable gaps.  Brownian-bridge limits for
$L^1$ empirical-process and Wasserstein functionals are classical; see del
...
Theorem~4.3 of Bourn and Erickson~\cite{BournErickson}: setting their total
...
independent conditional decomposition, obtained from the symmetric-difference
...
identified by the Git tag \texttt{paper-2026-09-01-r1} at
\url{https://github.com/skypher/matchgate/tree/paper-2026-09-01-r1}.  It is
...
The \texttt{check-exact-mean} target first checks
...
Discrete Mathematics \textbf{348} (2025), 114336.
