...
\title{Brownian-bridge scaling of distances in\\
matchgate commutator graphs}
...
\newcommand{\shorttitle}{BROWNIAN-BRIDGE SCALING IN MATCHGATE GRAPHS}
...
\begin{abstract}
We determine the distributional asymptotics and mean of the uniform pairwise
distance along every sequence of matchgate commutator-graph components
satisfying $\kappa_n/(2n)\to\rho\in(0,1)$.  If $S$ and $T$ are independent
uniform $\kappa_n$-subsets of $[2n]$, the graph distance is
$D_n=\sum_i|s_i-t_i|$ and
\[
 \frac{D_n}{n^{3/2}}
...
\]
After averaging jointly over a uniform initial Pauli vertex and an independent
Haar matchgate unitary, the graph-complexity mean has the same asymptotic.  We
also prove that graph complexity is a pointwise lower bound for Krylov
complexity under a specified Hamiltonian evolution.
...
the limiting law and its expectation constant, and then to transfer the mean
asymptotic to graph complexity.  That transfer averages first over a Haar
matchgate unitary and then over the initial Pauli vertex; it is not a
pointwise-in-vertex scaling statement.
...
path~\cite{FabilaMonroy}, also called a symmetric power of a
path~\cite{AudenaertEtAl}.  This terminology predates the matchgate
application, and the ordered-coordinate distance formula appears, for example,
in Ibarra and Rivera~\cite[Lemma~5.2]{IbarraRivera}.  The transport
...
\begin{proof}
Define the Jordan--Wigner Majorana operators by
\[
 c_{2j-1}=Z_1\cdots Z_{j-1}X_j,\qquad
 c_{2j}=Z_1\cdots Z_{j-1}Y_j\qquad(1\leq j\leq n).
\]
They satisfy $c_a^\dagger=c_a$ and
$\{c_a,c_b\}=2\delta_{ab}I$.  In this basis,
\[
 Z_j=-i c_{2j-1}c_{2j},
 \qquad
...
Moreover, products of the adjacent exchanges above act transitively, up to
phases of modulus one, on the degree-$k$ Majorana monomials, which are the
Pauli strings in $\mathcal C_k$.  This transitivity is the only
representation-theoretic input below.  In particular, the argument does not
assume that the degree-$k$ Majorana representation is irreducible; this matters
for the middle component $k=n$.
...
\eqref{eq:graph-complexity-definition} gives Eq.~(38) of West et al.:
\[
...
\begin{corollary}[Pointwise graph--Krylov comparison for generator-supported
Hamiltonians]\label{cor:krylov}
...
$P_h=\1_{\{h=0\}}+z(P_{h-1}+P_{h+1})$ and selecting the unique solution regular
at $z=0$.  Splitting a bridge at its marked time and reversing the segment
...
For the variable $M$ in \eqref{eq:M-law}, with $\mu=n/2$ and
$A_n:=\{|M-\mu|\leq n/4\}$,
for every $t\geq0$,
...
and, as $n\to\infty$,
\begin{align}
 \E\left[\sqrt M\,;\,A_n\right]
 &=\sqrt{\frac n2}\left(1-\frac1{16n}+O(n^{-2})\right),
 \label{eq:sqrt-M-expansion}\\
 \E\left[M^{-1/2}\,;\,A_n\right]
 &=\sqrt{\frac2n}\left(1+O(n^{-1})\right).
 \label{eq:inverse-sqrt-M-expansion}
\end{align}
Here $\E[M^{-1/2};A_n]$ denotes integration of $M^{-1/2}$ over $A_n$; it is
well-defined because $M\geq n/4>0$ on $A_n$.
\end{lemma}

\begin{proof}
The law of $M$ is sampling without replacement from a population of $n$ zeros
and $n$ ones.  Hoeffding's convex comparison
\cite[Theorem~4]{Hoeffding}, stated as Theorem~3.1 by
...
\end{equation}
On $A_n$, Taylor's formula for $x^{1/2}$ through degree three
...
\url{https://github.com/skypher/matchgate}.  The manuscript version is archived
by the immutable Git tag \texttt{paper-2026-08-31} at
\url{https://github.com/skypher/matchgate/tree/paper-2026-08-31}.  It is built
...
compiler with OpenMP support, and the checker requires Python~3.  The values in
...
Journal of Combinatorial Theory, Series B \textbf{97} (2007), 74--90.
...
and D. R. Wood,
\emph{Token graphs},
Graphs and Combinatorics \textbf{28} (2012), 365--380.
...
Physical Review X \textbf{9} (2019), 041017.
