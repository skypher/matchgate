...
\hypersetup{
  ...
  pdftitle={Brownian-Bridge Scaling of Distances in Matchgate Commutator Graphs},
...
\begin{abstract}
We determine the distributional asymptotics and mean of the distance between
two independent uniformly sampled vertices, for every sequence of matchgate
commutator-graph components satisfying
$\kappa_n/(2n)\to\rho\in(0,1)$.  If $S$ and $T$ are the corresponding uniform
$\kappa_n$-subsets of $[2n]$, the graph distance is
...
Haar matchgate unitary, the graph-complexity mean is
$\frac{\sqrt{2\pi}}2\sqrt{\rho(1-\rho)}\,n^{3/2}+o(n^{3/2})$.  We also prove
that graph complexity is a pointwise lower bound for Krylov complexity under a
specified Hamiltonian evolution.
...
\cite{Vallender,DeAngelisGray}.  Brownian-bridge limits for $L^1$
empirical-process and Wasserstein functionals are classical; see, for example,
del Barrio, Gin\'e, and Matr\'an~\cite{delBarrioGineMatran}.
...
Moreover, products of the adjacent exchanges above act transitively on the
degree-$k$ Majorana monomials.  Up to phases of modulus one, these monomials
are the Pauli strings in $\mathcal C_k$.  This transitivity is the only
...
\]
Averaging also over $P_n$ gives exactly $\E D_{2n,k_n}$, so
...
\end{align}
Here $\E[M^{-1/2};A_n]=\E[M^{-1/2}\1_{A_n}]$; it is well-defined because
$M\geq n/4>0$ on $A_n$.
\end{lemma}
...
\url{https://github.com/skypher/matchgate}.  This revised manuscript is
identified by the Git tag \texttt{paper-2026-08-31-r1} at
\url{https://github.com/skypher/matchgate/tree/paper-2026-08-31-r1}.  It is
built and run, for example, with
\begin{gather*}
