...
\hypersetup{
  ...
  pdftitle={Matchgate Commutator Graphs as Rectangular Minuscule Lattices},
...
\title{Matchgate commutator graphs as rectangular minuscule lattices:\\
Brownian-bridge distances and complexity}
...
\begin{abstract}
Matchgate commutator-graph components are Hasse graphs of rectangular
minuscule lattices.  Defant, F\'eray, Nadeau, and Williams previously proved
...
formula~\cite{DefantEtAl}.  We give an
independent finite-population derivation valid for every sequence of matchgate
components satisfying $\kappa_n/(2n)\to\rho\in(0,1)$ and translate the result
to matchgate complexity.  If $S$ and $T$ are independent uniform
$\kappa_n$-subsets of $[2n]$, set
$D_n:=D_{2n,\kappa_n}(S,T)=\sum_i|s_i-t_i|$.  Then
\[
 \frac{D_n}{n^{3/2}}
...
$\E D_n\sim\sqrt{2\pi}\,n^{3/2}/4$, which resolves Conjecture~14 of West et
al. after the matchgate--minuscule-lattice identification.  Moment convergence
also gives
\[
 \Var(D_n)\sim\rho(1-\rho)
 \left(\frac{28}{15}-\frac\pi2\right)n^3.
\]
We give an independent symmetric-difference finite sum in the central
component; the published closed form yields the expansion
\[
...
identifies the token graph with the Hasse graph of
$\mathcal P_{k,N-k}$.  If $p_r$ is the height of the path for $S$ after $r$
steps, then $p_r=2C_S(r)-r$; hence the path-area distance used for rectangular
minuscule lattices is exactly \eqref{eq:matching-distance}.  Defant, F\'eray,
Nadeau, and Williams~\cite[Corollary~3 and Proposition~5]{DefantEtAl} give a
closed formula for its Wiener index and prove the fixed-aspect-ratio
Brownian-bridge limit in distribution and in moments.  Under the standard
unordered-pair convention for the Wiener index $W(G)$, the average used by
West et al. is $2W(G)/|V(G)|^2$.

For rectangles with a fixed aspect ratio, taking
$\alpha=\rho/(1-\rho)$ in the result of Defant et al. gives the same limiting
law, mean, and variance constants as those stated below; at $\alpha=1$ it gives
the central case.  Thus their theorem, combined with the matchgate component
identification of West et al., already implies Conjecture~14.  The purpose of
the present paper is to make this implication explicit, give an independent
finite-population proof valid for every sequence $k_n/(2n)\to\rho$, and
formulate the exact matchgate graph- and Krylov-complexity consequences.  The
Haar transfer averages first over a Haar
...
uniform integrability needed for moment convergence directly from
hypergeometric moment bounds.

The token-graph terminology is due to Fabila-Monroy et
al.~\cite{FabilaMonroy}; the same graph is also called a symmetric power of a
...
\cite{Vallender,DeAngelisGray}.  Expected one-dimensional earth mover distance
under a different, simplex-based sampling law is studied by Bourn and
Willenbring~\cite{BournWillenbring}.  Brownian-bridge limits for $L^1$
empirical-process and Wasserstein functionals are classical; see del Barrio,
Gin\'e, and Matr\'an~\cite{delBarrioGineMatran} and their correction to separate
heavy-tail statements~\cite{delBarrioCorrection}.  Those corrected statements
are not used here.
...
independent symmetric-difference identity for its central specialization, and
...
$\mathcal P_{k,N-k}$, and its graph distance is $D_{N,k}$.  When $N=2n$, it is
the matchgate component $\mathcal C_k$ with the metric in
...
Proposition~5 of Defant et al.~\cite{DefantEtAl} at rectangle aspect ratio one;
...
The adjacent transpositions generate the full symmetric group on the $2n$
Majorana labels.  Given two $k$-element supports, choose a permutation carrying
the first support to the second and express it as a product of adjacent
transpositions.  The corresponding product of the $V_a$ maps the first
degree-$k$ Majorana monomial to the second up to a phase of modulus one.
These monomials are, up to such phases, the Pauli strings in $\mathcal C_k$.
This transitivity is the only
...
\begin{proposition}[Published rectangular-lattice mean]\label{prop:closed-form}
For independent uniform $S,T\in\Omega_{N,k}$,
...
The central component also has an independent symmetric-difference
decomposition.  Let $W^{(m)}$ be a
...
\begin{proof}
The Stirling expansion with remainder
\[
...
The matchgate components of West et al. are rectangular minuscule lattices, so
...
and explicit moment bounds.  The published Wiener-index formula supplies the
closed finite-size mean and its leading $n^{1/2}$ correction, while the central
symmetric-difference decomposition gives an independent finite-sum identity.
...
identified by the Git tag \texttt{paper-2026-08-31-r2} at
\url{https://github.com/skypher/matchgate/tree/paper-2026-08-31-r2}.  It is
...
\texttt{check-exact-mean} target first checks
Propositions~\ref{prop:graph-metric}, \ref{prop:closed-form}, and
\ref{prop:exact-computation} and Lemma~\ref{lem:transport} by exact rational
...
Algebraic Statistics \textbf{11} (2020), 53--78.
...
Electronic Journal of Combinatorics \textbf{31} (2024), Paper No.~P1.41.
...
Annals of Probability \textbf{31} (2003), 1142--1143.
\href{https://doi.org/10.1214/aop/1048516548}{doi:10.1214/aop/1048516548}.
