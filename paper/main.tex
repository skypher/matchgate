...
\]
and, in the central component, an exact finite sum and its first correction
$-5\sqrt{2\pi}\,n^{1/2}/64$.  We also record the resulting graph-complexity
statement, averaged jointly over a uniform initial Pauli vertex and an
independent Haar matchgate unitary, and the pointwise Krylov lower bound for a
...
If $\mu_S=k^{-1}\sum_{i=1}^k\delta_{s_i}$ and
$\mu_T=k^{-1}\sum_{i=1}^k\delta_{t_i}$, then
...
mean $p$ and $\psi(\lambda)=\log\E\exp(\lambda(X-p))$, then
$\psi(0)=\psi'(0)=0$ and
...
\]
Proposition~7 of West et al.~\cite{WestEtAl} is stated using the unnormalized
Hilbert--Schmidt pairing.  Together with invariance of Haar measure under
$U\mapsto U^\dagger$, it gives
\[
 \E_U\left|\operatorname{Tr}(Q^\dagger U^\dagger P_nU)\right|^2
 =\frac{2^{2n}}{|\mathcal C_{k_n}|}.
\]
Thus $\E_U|c_Q(U)|^2=|\mathcal C_{k_n}|^{-1}$.  Substitution in
...
and contain $2n-2m$ inactive sites in total.  Hence
\[
...
enumerating pairs of subsets.  If $H_{N,k,r}$ is hypergeometric with population
size $N$, $k$ selected sites, and an initial segment of length $r$, and
$H'_{N,k,r}$ is an independent copy, then
...
The proof architecture was developed with assistance from OpenAI GPT-5.6 Sol
(\texttt{gpt-5.6-sol}) at max reasoning effort.  The model served as an
...
weights to evaluate the exact finite-sum identities numerically.
