...
\title{Brownian-Bridge Scaling of Graph Complexity in Matchgate Dynamics}
...
$-5\sqrt{2\pi}\,n^{1/2}/64$.  We also record the resulting Haar-averaged
graph-complexity statement and the pointwise Krylov lower bound for a specified
...
\begin{proof}
Equation~(38) of West et al.~\cite{WestEtAl} states, for fixed $P_n$, that
the Haar expectation of graph complexity is the mean of
$d_{\mathcal C_{k_n}}(P_n,Q)$ over uniform $Q\in\mathcal C_{k_n}$.  Averaging
also over $P_n$ gives exactly $\E D_{2n,k_n}$.  Apply
...
\]
The Lanczos construction gives
$O_j\in\operatorname{span}\{P,\mathcal L_HP,\ldots,\mathcal L_H^jP\}$
before termination, hence $\mathcal K_j\subseteq\mathcal F_j$ for every
$j\geq0$.  The full cyclic Krylov space is invariant under $\mathcal L_H$;
therefore $P_t=e^{it\mathcal L_H}P$ belongs to
$\mathcal K_{d_K-1}$.
